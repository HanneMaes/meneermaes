...
fs.readFile("settings-maesbot.json", "utf8", (err, data) => {
  // Read the JSON file
  if (err) {
  ...
  try {
    ...
    fs.readFile(
      path.join(
        jsonObjectSettings["private-data-dir"],
        jsonObjectSettings["klassen en leerlingen"],
      ),
      "utf8",
      (err, data) => {
        // Read the JSON file
...
function showMenu(settingsJson, klassenJSON) {
  ...
  console.log();
  console.log(color.info("Offline markdown"));
  console.log(color.answer(" 6. Open diff dir"));
  console.log(color.answer(" 7. MD 2 PDF"));
  console.log();
...
function openDir(directory) {
  ...
  if (platform === "linux") {
    exec(`xdg-open "${directory}"`, (err) => {
      if (err)
        console.error(
          "maesbot.js openDir() -> Error opening directory on Linux:",
          err,
        );
    });
    ...
    exec(`open "${directory}"`, (err) => {
      if (err)
        console.error(
          "maesbot.js openDir() -> Error opening directory on macOS:",
          err,
        );
    });
    ...
    exec(`start "${directory}"`, (err) => {
      if (err)
        console.error(
          "maesbot.js openDir() -> Error opening directory on Windows:",
          err,
        );
    });
  } else {
    console.error(
      "maesbot.js openDir() -> Unsupported operating system:",
      platform,
    );
  }
}
...
function selectFromArray(inputArray) {
  try {
    ...
    console.error(
      "maesbot.js selectFromArray() -> canceled or error occurred.",
    );
